...
year_slider = Slider(fig[2, 1], range=1989:2023, startvalue=2000)
Label(fig[3, 1], "Year", tellwidth=false)
dataset_menu = Menu(fig[4, 1], options=["Emissions", "Deforestation"])
Label(fig[5, 1], "Dataset", tellwidth=false)
on(year_slider.value) do v
    year[] = v
    update_plot()
end
on(dataset_menu.selection) do v
    selected_dataset[] = v
    update_plot()
end
update_plot()
animate_button = Button(fig[3, 2], label="Animate")
